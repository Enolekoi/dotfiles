vim.g.mapleader = ' '
-- Remaps --
vim.keymap.set('n', '<leader>pv', vim.cmd.Ex, {desc = 'Open Netrw'})                             -- keybind to open Netrw

vim.keymap.set('v', 'J', ':m ">+1<CR>gv=gv', {desc = 'Move visual selection down'})              -- Move visual selection down
vim.keymap.set('v', 'K', ':m "<-2<CR>gv=gv', {desc = 'Move visual selection up'})                -- Move visual selection up
...
vim.keymap.set('x', '<leader>p', '\'_dP', {desc = 'pasting (keeping buffer)'})                   -- content of yanked 'foo' stays in the buffer when pasting of selected 'bar'

vim.keymap.set('n', '<leader>y', '\'+y', {desc = 'yank to system clipboard'})                    -- use <leader>y to yank to system clipboard
vim.keymap.set('v', '<leader>y', '\'+y', {desc = 'yank to system clipboard'})                    -- use <leader>y to yank to system clipboard
vim.keymap.set('n', '<leader>Y', '\'+Y', {desc = 'yank to system clipboard'})                    -- use <leader>Y to yank to system clipboard

vim.keymap.set('n', '<leader>d', '\'_d', {desc = 'delete to void'})                              -- delete to void
vim.keymap.set('v', '<leader>d', '\'_d', {desc = 'delete to void'})                              -- delete to void
...
vim.keymap.set('n', '<leader>lf', function()                           -- use the current lsp server for formating
  vim.lsp.buf.format()
end, {desc = 'Use LSP for formatting'})
...
vim.keymap.set('n', '<leader>s', [[:%s/\<<C-r><C-w>\>/<C-r><C-w>/gI<Left><Left><Left>]], {desc = 'replace word across file'}) -- Replace current word across the file
